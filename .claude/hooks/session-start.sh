...
# graphify knowledge graph — remind Claude to query before reading files
if [ -f "$CLAUDE_PROJECT_DIR/graphify-out/graph.json" ]; then
  echo "graphify: graph ready at graphify-out/graph.json (10.7x token reduction). Use /graphify query \"...\" before reading multiple files. Run /graphify --update after significant changes."
fi
#
