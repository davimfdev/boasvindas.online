...
|---|---|
| CSS | **Tailwind CSS v4** — use CSS vars for theming, no raw values |
| Primitives | **shadcn/ui** — extend existing components, don't add competing libs |
| Animation | **Framer Motion** — for builder transitions and landing page effects |
| Icons | **Lucide React** |
| State | **Zustand** — global state; TanStack Query for server state |
| Forms | **React Hook Form + Zod** — share Zod schemas from `packages/types` |
