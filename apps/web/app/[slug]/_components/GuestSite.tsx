...
export function GuestSite({ title, whatsapp }: GuestSiteProps) {
  ...
  return (
    <div className="guest-site min-h-screen flex flex-col bg-[#f0fdfa] text-[#134e4a] relative">
      <header
        className={`sticky top-0 z-50 px-5 py-4 flex items-center justify-center transition-all duration-300 border-b ${
          isHome
            ? scrolled ? 'bg-[#0d9488]/95 backdrop-blur-md shadow-lg border-[#0d9488]/20' : 'bg-[#0d9488] border-transparent'
        ...
        <div className="w-full max-w-5xl flex items-center justify-between">
          <div className="flex items-center gap-2">
            {!isHome && (
              <button
                ...
                className="p-1.5 hover:bg-teal-50 rounded-full transition-colors text-[#0d9488]"
                aria-label="Voltar para o início"
              >
                <ChevronLeft size={24} />
              </button>
            )}
            <h1 className={`font-serif font-bold transition-all duration-300 flex flex-col items-start ${isHome ? 'text-[#fbbf24]' : 'text-[#0d9488]'}`}>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`p-2 rounded-full transition-colors ${isHome ? 'text-white hover:bg-white/10' : 'text-[#0d9488] hover:bg-teal-50'}`}
      ...
      {isQrOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-[#134e4a]/60 backdrop-blur-sm" onClick={() => setIsQrOpen(false)} />
          <div className="relative bg-white rounded-3xl shadow-2xl p-8 flex flex-col items-center gap-4 max-w-xs w-full animate-slideUp">
            <button onClick={() => setIsQrOpen(false)} className="absolute top-4 right-4 text-gray-300 hover:text-[#0d9488]" aria-label="Fechar">
              <X size={24} />
            </button>
            <h2 className="font-serif font-bold text-lg text-[#0d9488]">Acesse no celular</h2>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] flex justify-end">
          <div className="absolute inset-0 bg-[#134e4a]/60 backdrop-blur-sm" onClick={() => setIsMenuOpen(false)} />
          <div className="relative w-full max-w-[350px] bg-white h-full shadow-2xl p-8 flex flex-col gap-8 animate-slideInRight">
            <div className="flex justify-between items-center border-b pb-6 border-gray-100">
              <h2 className="text-2xl font-serif font-bold text-[#0d9488]">Menu</h2>
              <button onClick={() => setIsMenuOpen(false)} className="text-gray-300 hover:text-[#0d9488]"><X size={28} /></button>
            </div>
            <nav className="flex flex-col gap-3 overflow-y-auto">
              <button
                onClick={() => setActiveSection('home')}
                className={`text-left p-4 rounded-2xl transition-all flex items-center gap-4 ${isHome ? 'bg-[#0d9488] text-[#fbbf24] font-bold shadow-lg' : 'text-[#134e4a] hover:bg-[#f0fdfa]'}`}
              >
                Início
              </button>
              {MENU_ITEMS.map((item) => (
                <button
                  ...
                  className={`text-left p-4 rounded-2xl transition-all flex items-center gap-4 ${activeSection === item.id ? 'bg-[#0d9488] text-[#fbbf24] font-bold shadow-lg' : 'text-[#134e4a] hover:bg-[#f0fdfa]'}`}
                >
                  <span className={`${activeSection === item.id ? 'text-[#fbbf24]' : 'text-[#0d9488]'} opacity-80`}>{item.icon}</span>
                  {item.title}
                </button>
              ))}
            </nav>
      ...
      <footer className="w-full bg-white border-t border-gray-100 py-12 px-6 mt-auto flex flex-col items-center">
        <div className="w-full max-w-5xl flex flex-col items-center text-center gap-6">
          <div className="flex flex-col gap-1">
            <p className="text-[#0d9488] font-serif font-bold text-xl uppercase tracking-tight">{title}</p>
          ...
          {whatsappUrl && (
            <a
              ...
              className="bg-[#25D366]/10 text-[#0d9488] py-2 px-4 rounded-xl hover:bg-[#25D366]/20 transition-all font-bold inline-flex items-center justify-center gap-2"
            >
      ...
      {!isHome && (
        <button
          onClick={() => setActiveSection('home')}
          className="fixed bottom-8 left-8 bg-[#0d9488] text-[#fbbf24] p-5 rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all z-40 border-4 border-white flex items-center justify-center animate-slideUp"
